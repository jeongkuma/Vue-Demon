...
export default {
  ...
  created() {
    ...
    this.dataObj.solMsgMSeq = this.data.solMsgMSeq
  ...
  methods: {
    ...
    getTargetSolSvrList() {
      ...
      requestApi({
        ...
        callback: res => {
          ...
          this.dataObj.targetSolSvrSeq = res.data.targetSolSvrList[0].solSvrSeq
        }
      })
    },
    getTranCdList() {
      getCommonCodeByParentCdId({ xMid: this.xMid, parentCdId: 'GN00000235' })
        .then(data => {
          ...
          this.dataObj.tranCd = data[0].cdId
        })
        .catch(e => {
          console.error(e)
        })
    ...
    getProtocolList() {
      getCommonCodeByParentCdId({ xMid: this.xMid, parentCdId: 'GN00000232' })
        .then(data => {
          ...
          this.dataObj.protocol = data[0].cdId
        })
        .catch(e => {
          console.error(e)
        })
    ...
    getMethodList() {
      getCommonCodeByParentCdId({ xMid: this.xMid, parentCdId: 'GN00000238' })
        .then(data => {
          ...
          this.dataObj.method = data[0].cdId
        })
        .catch(e => {
          console.error(e)
        })
    ...
    chkValid() {
      ...
      if (!this.uriDuplChk && this.dataObj.tranCd !== 'GN00000236') {
      ...
      this.confirm(this.$t('createSolMsg'), this.insertSolMsgInfo)
    },
    fnNumChk(num) {
      if (num !== null && num !== '') {
        return /^[0-9]*$/.test(num)
    ...
    insertSolMsgInfo() {
      requestApi({
        url: '/online/solsvrmsg/insertSolMsgInfo',
        headers: {
          'X-MID': this.xMid
        },
        method: 'post',
